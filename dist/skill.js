...
function stringProp(source, key, fallback) {
    const value = source[key];
    return typeof value === 'string' ? value : fallback;
}
function numberProp(source, key, fallback) {
    const value = source[key];
    return typeof value === 'number' && Number.isFinite(value) ? value : fallback;
}
function booleanProp(source, key) {
    const value = source[key];
    return typeof value === 'boolean' ? value : undefined;
}
function validateColumn(value, index) {
    if (!isRecord(value)) {
...
export function generateSkill(cols, name = 'json2mdt-skill') {
    return {
        name,
...
export function saveSkill(skill, path) {
    writeFileSync(path, `${JSON.stringify(validateSkill(skill), null, 2)}\n`, 'utf8');
}
export function renderSkillHeader(skill) {
